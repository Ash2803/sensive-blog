...
def index(request):
    popular_posts = Post.objects.annotate(Count('likes')).order_by('-likes__count')[:5].prefetch_related('author')
    ...
    popular_tags = Tag.objects.annotate(Count('posts')).order_by('-posts__count')[:5]
